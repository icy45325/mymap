...
**台阶 2.5 —— 交换日记·带外版（⚠️ 已被 v3 取代，2026-07-11）**
> 本节描述的「整本口令互寄 + 多人群本」实现已下线（git 历史可找回，其传输层留给未来远程模式）。
> 现行规格 = **交换日记 v3**：本=旅程×对象 / 页=足迹 / 页级揭晓 / 火漆封印 / **本机传递**（零后端全离线）/
> 文字·涂鸦·语音，见 [`DESIGN-exchange-diary.md`](DESIGN-exchange-diary.md)。以下保留作历史记录：
