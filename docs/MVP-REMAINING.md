...
| A2 | 护照本完善 | ✅ | 整页放大/资料页/入境章**逐国暗纹**均已落地 |
...
| N14 | **地区特色邮票**：12 国原生绘制，点亮过即解锁，寄卡可选 | 邮票选择器出现已解锁国家款；接收方正确显示 |
| N15 | **收集图鉴**（Me 页入口）：地区邮票/节日章/基础款成册 | 解锁态正确；进度环计数 |
| N16 | **地图皮肤（Plus）**：霓虹/极光/暖阳 | 非 Plus 弹 Paywall；切换后点阵+真实地图变色 |
| N17 | **自定义 App 图标（Plus）**：暗金/粉霓 | 非 Plus 弹 Paywall；切换后桌面图标生效 |
| N18 | 体验打磨：护照逐国暗纹、时间线/徽章墙入场动效、翻面回弹 | 护照章详情、滚动列表手感 |
| N19 | **统一主题**：高亮色+地图+App 图标一套切换（霓虹/极光/暖阳），替代地图皮肤 | Plus 切主题→全 App 高亮/地图/图标一起变；非 Plus 弹 Paywall |
| N20 | **小组件改 Plus 专属**：非 Plus 显示锁定态 | 未购买时三种小组件均显示锁定；购买后自动恢复 |
| N21 | **地区邮票仅限本国足迹**（不再全部已解锁国家可选） | 日本足迹只见日本票；他国足迹不见 |
| N22 | **节日章改寄卡可选**：窗口 ±5 天+地区限定，随口令显式传达（删收件方自动替换） | 窗口内出现可选；窗口外不出现；收件方看到所选章 |
| N23 | **图鉴详情弹窗**：点邮票/节日章/邮戳→半屏说明（解锁/使用限制/用法） | 各类条目弹窗内容正确 |
| N24 | **典藏邮票（Plus）**：14 枚（CN3/US3/JP4/AE4，AE 绑酋长国），非 Plus 露 2 枚带锁作付费入口 | 日本足迹见 4 枚日本票（Plus）；迪拜足迹只见哈利法塔；非 Plus 点锁票弹 Paywall |
...
- 视频上传 / 明信片送达统计 / RevenueCat → 后续。（地图皮肤已提前落地，见 N16）
